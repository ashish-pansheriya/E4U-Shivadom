...
from .models import CITY_CHOICES, FILM_ROLE_CHOICES
...
class Searchview(View):
    def get(self, request, *args, **kwargs):
        ...
        # Improved search query with multiple fields
        if query:
        ...
        # Pagination
        paginator = Paginator(queryset, BLOG_POST_PER_PAGE)
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return render(request, 'friends/partials/results.html', {
                'queryset': queryset,
                'query': query
            })
            
        return render(request, 'friends/friends_home.html', {
            'queryset': queryset,
            'query': query,
            'gender': gender,
            'city': city,
